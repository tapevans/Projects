...
dSVdt_0 = np.zeros_like(SV_0)

        #print(SV_0)
        #SV_0 = np.reshape(SV_0, (1, -1))
SV_0 = SV_0.flatten()
dSVdt_0 = dSVdt_0.flatten()
        #print(SV_0)
...
pos_vec_minus_half = np.zeros(N_tot)
pos_vec_plus_half = np.zeros(N_tot)
...
for i in range(N_tot):
    if i < (N_an + N_sep):
        if i < N_an:
            ...
            pos_vec_plus_half[i] = pos_vec[i] + 0.5*del_z_an
            pos_vec_minus_half[i] = pos_vec[i] - 0.5 * del_z_an
            del_z_vec[i] = del_z_an
            D_eff_Li_ion_vec[i] = D_eff_li_ion_an
        else:
            pos_vec[i] = (i-N_an) * del_z_sep + 0.5 * del_z_sep + H_an
            pos_vec_plus_half[i] = pos_vec[i] + 0.5 * del_z_sep
            pos_vec_minus_half[i] = pos_vec[i] - 0.5 * del_z_sep
            del_z_vec[i] = del_z_sep
            D_eff_Li_ion_vec[i] = D_eff_li_ion_sep
    else:
        pos_vec[i] = (i - N_an - N_sep) * del_z_ca + 0.5 * del_z_ca + H_an + H_sep
        pos_vec_plus_half[i] = pos_vec[i] + 0.5 * del_z_ca
        pos_vec_minus_half[i] = pos_vec[i] - 0.5 * del_z_ca
...
class pars:
    ...
    A_geo_min = A_geo_min
    pos_vec = pos_vec
    pos_vec_plus_half = pos_vec_plus_half
    pos_vec_minus_half = pos_vec_minus_half
    del_z_vec = del_z_vec
